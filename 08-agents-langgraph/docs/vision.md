...
│   ├── agent.py                # Создание ReAct агента через create_agent() (новое)
│   ├── tools.py                # Инструмент rag_search для агента (новое)
...
3. **agent.py** - ReAct агент (новое)
   - `create_bank_agent()` - создание агента через `create_agent()` из LangChain 1.0
   - Использует ChatOpenAI модель и инструмент rag_search
   ...
   - Агент самостоятельно принимает решения о вызове инструмента
   - Глобальная переменная `bank_agent` для хранения экземпляра агента

4. **tools.py** - инструменты агента (новое)
...
bank_agent (ReAct цикл):
    1. Think (Reason) - агент анализирует вопрос
    2. Act - если нужна информация:
       ├─ вызов tools.py::rag_search(query) →
       │  rag.py::retrieve_documents() →
       │  retriever (semantic/hybrid/hybrid_reranker) →
       │  найденные документы →
       │  возврат контекста агенту
       └─ агент формирует ответ на основе контекста
    3. End - если поиск не нужен, агент отвечает напрямую
