...
def init_ragas_metrics():
    ...
    # Уменьшаем параллелизм до 1, чтобы избежать rate limit ошибок
    run_config = RunConfig(
        max_workers=1,  # Обрабатываем по одному примеру за раз
        max_wait=300,   # Увеличиваем время ожидания
...
def evaluate_dataset(dataset_name: Optional[str] = None) -> Dict[str, Any]:
    ...
    def target(inputs: dict) -> dict:
        ...
        # Используем sync invoke - RAG цепочка поддерживает оба варианта
